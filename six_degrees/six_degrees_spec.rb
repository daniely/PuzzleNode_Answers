...
describe SixDegrees do
  describe ".connect" do
    it 'raises error if no filename or text specified' do
      expect do
        SixDegrees.connect()
    ...
    it 'works with filename param' do
      result = { 'alberta'  => ['bob', 'christie', 'duncan'], 
                 ...
                 'farid'    => ['duncan'] }
      SixDegrees.connect(:filename => 'sample_input.txt').should == result
    end
...
EOS
      result = { 'alberta'  => ['bob', 'christie', 'duncan'], 
                 ...
                 'emily'    => ['christie', 'duncan'], 
                 'farid'    => ['duncan'] }
      SixDegrees.connect(:text => text).should == result
    end
...
=begin
describe "put it all together" do
  it 'do' do
    c = Connect.connections('complex_input.txt')
    result = ''

    c.each do |k, v|
      levels = Connect.bfs(c, k)
    ...
    #expected = File.read('sample_output.txt')
